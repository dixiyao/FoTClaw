...
MAX_OPENCLAW_MESSAGE_CHARS = int(os.environ.get("FOTCLAW_MAX_MSG_CHARS", "8000"))
...
def parse_openclaw_agent_args(raw_args: list[str]) -> dict[str, Any]:
    ...
    if not message:
        raise OpenClawError(
            "FoTClaw requires a prompt message. Pass the same arguments you would after "
...
def _run_command(
    ...
    *,
    ...
    try:
        ...
        raise OpenClawError(
            "OpenClaw CLI not found. Install it or set OPENCLAW_PATH/FOTCLAW openclaw_path."
        ) from exc
...
def resolve_default_model() -> str | None:
    env_model = os.environ.get("FOTCLAW_DEFAULT_MODEL")
    if env_model:
...
def render_insight_markdown(insights: dict[str, str]) -> str:
    lines = [
        "# Insight Library",
        "",
        "This file is generated by FoTClaw from aggregated reasoning traces.",
        "",
    ]
